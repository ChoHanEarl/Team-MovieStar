    ...
    useEffect(() => {
        const fetchFavoriteMovies = async () => {
            if (!favoriteMovies || favoriteMovies.length === 0) {
                console.log("찜 목록이 비어 있습니다");
                return;
            }
            try {
                const movieId = favoriteMovies[0]?.id;
                const response = await axios.get(`http://Moviestar-env.eba-7mxrpygu.ap-northeast-2.elasticbeanstalk.com/movie/${movieId}`);
                if (response.data) {
